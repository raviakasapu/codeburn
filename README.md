...
|----------|---------------|-------|
| **Bahulam Code** | `~/.bahulam/projects/<project-slug>/<session-id>.jsonl` (honors `BAHULAM_PROJECTS_DIR`, then `BAHULAM_HOME/projects`) | Reads Bahulam JSONL events. `complete` events provide per-turn token usage and reported cost; multi-model usage emits one call per model; non-root model roles populate subagent breakdowns; `tool_call` / `tool_request` events attach tool and shell-command usage to the following turn. |
